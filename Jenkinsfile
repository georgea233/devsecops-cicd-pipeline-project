...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit george233/reddit:latest "
                       sh "docker push george233/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image george233/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.137.140.92 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.133.119.116:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#cit-devsecops-gka', //update and provide your channel name
